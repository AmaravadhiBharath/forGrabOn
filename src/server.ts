...
server.tool(
  "terminate_campaign",
  "EMERGENCY KILL SWITCH: Immediately deactivates all variants across all 6 channels. Use this when merchant stock hits zero or for urgent campaign withdrawals.",
  {
    campaign_id: z.string().describe("The ID or reference of the campaign to terminate (e.g. Zomato-Food-123)"),
    reason: z.string().describe("Reason for termination (e.g. 'Stock Exhaustion', 'Merchant Request')"),
  },
  async (args) => {
    const channels: string[] = ["email", "whatsapp", "push", "glance", "payu", "instagram"];
