import { CopyVariant, Language, VariantKind } from "./types";

function localizeLineToHindi(text: string, variant: VariantKind): string {
...
function localizeLineToTelugu(text: string, variant: VariantKind): string {
  if (variant === "urgency") {
    return text
...
export function expandWithLocalizations(
  englishVariants: CopyVariant[]
): CopyVariant[] {
  const all: CopyVariant[] = [];
