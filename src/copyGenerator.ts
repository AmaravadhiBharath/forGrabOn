import { Channel, CopyVariant, DealInput, Language, VariantKind } from "./types";

const VARIANTS: VariantKind[] = ["urgency", "value", "social_proof"];
...
function formatDiscount(deal: DealInput): string {
...
function expiryPhrase(deal: DealInput): string {
  ...
  if (diffDays <= 0) return "today only";
...
function minOrderPhrase(deal: DealInput): string {
  if (!deal.min_order_value || deal.min_order_value <= 0) return "";
  return `on orders above ₹${deal.min_order_value}`;
}

function exclusivityPhrase(deal: DealInput): string {
...
export function generateEnglishVariants(deal: DealInput): CopyVariant[] {
